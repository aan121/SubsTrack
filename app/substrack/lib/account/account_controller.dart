...
class AccountController extends GetxController{
  ...
  Future<void> updateUserData() async {
    try {
      await database.child(Extra.CURRENT_USER_ID).update({
        'userEmail': emailController.value.text,
        'userFirstName': firstNameController.value.text,
        'userLastName': lastNameController.value.text,
        'userPhoneNumber': phoneController.value.text,
      });
      print('User data updated successfully');
      Get.snackbar("Successfully", "data updated", backgroundColor: ThemeColors.tabColor);
    } catch (error) {
